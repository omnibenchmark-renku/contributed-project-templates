...
meta <- data.frame("batch_id" = sce$patient,
...
# Dataset info file (optional)
# Json file with the following variables: 
# url, tissue, n_cells, n_genes, n_cluster, n_batches, description
info_list <- list(
    "url" = "https://www.biorxiv.org/content/10.1101/2020.12.11.420885v1",
    "tissue" = "cerebrosial spine fluid (csf)",
    "n_cells" = ncol(sce),
    "n_genes" = nrow(sce),
    "n_cluster" = length(levels(as.factor(sce$ident))),
    "n_batches" = length(levels(as.factor(sce$patient))),
    "description" = "csf sample from 2 individuals without replicates.\n
                     A clear batch effect is associated to the patient_id.\n
                     Clustering, but no cell type annotations exist.",
    "update" = "FALSE"
)
